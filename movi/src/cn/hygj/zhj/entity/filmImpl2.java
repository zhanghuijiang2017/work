...
public class filmImpl2 implements baseDao {
	...
	public String findAll(Object obj){
		...
		conn=DBhelp.getconnection();
		try {
			System.out.println("select * from (select f.*,rownum r from movie_film f where (?='null' or ftype like '%"+type+"%') and (?='null' or fcountry like '%"+country+"%') and (?=1 or fmtime='"+time+"')) where r>? and r<=?");
